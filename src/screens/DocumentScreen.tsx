...
function ReadView({ documentReading, topic, hasCache, profile, extractedText, courseStreaming, subStatuses, onSubStatusChange, focusChatMessages, onFocusChatMessagesChange, understandTexts, onUnderstandTextsChange, understandLoading, onUnderstandLoadingChange, understandTextsLoaded, topicVisuals, onTopicVisualsChange, topicVisualLoading, onTopicVisualLoadingChange, onBack, onPractice, onRegenerate, onPodcast, weakSpots = {}, initialTab = 'understand' }: {
  ...
  // ── Read Aloud state ─────────────────────────────────────────
  type RaPhase = 'idle' | 'preloading' | 'loading' | 'playing' | 'paused';
  const [raPhase,         setRaPhase]         = useState<RaPhase>('idle');
  const [raPreloadReady,  setRaPreloadReady]  = useState(0);  // topics pre-generated so far
  ...
  const raStop = useCallback(() => {
    ...
    raUrlRef.current = '';
    setRaPhase('idle'); setRaTopicIdx(0); setRaWordIdx(-1); setRaPreloadReady(0);
  }, []);
  ...
  raPlayRef.current = async (idx: number) => {
    ...
    // No prefetch needed — all audio was pre-generated by raStart()
  ...
  const raStart = () => {
    ...
    const texts  = raTextsRef.current;
    setRaPreloadReady(0);
    setRaPhase('preloading');

    // Generate all topic audio in parallel — fire all requests simultaneously
    // so total wait ≈ one topic (~5s) regardless of topic count.
    const tasks = topics.map(async (t, idx) => {
      // Already cached from a previous session — count as instantly ready
      if (raCacheRef.current.has(idx)) {
        setRaPreloadReady(prev => prev + 1);
        return;
      }
      const raw = texts[t.topicId];
      if (!raw) { setRaPreloadReady(prev => prev + 1); return; } // no summary — skip
      const plain = stripMd(raw);
      const words = plain.split(/\s+/).filter(Boolean);
      try {
        const base64 = await generateSpeech(plain);
        raCacheRef.current.set(idx, { url: pcmToWavUrl(base64), words });
      } catch { /* skip failed topics — playback will skip them too */ }
      setRaPreloadReady(prev => prev + 1);
    });

    void Promise.all(tasks).then(() => {
      void raPlayRef.current!(0);
    });
  };
    ...
    <div style={{ marginBottom: 20 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        ...
        {notesMode === 'notes' ? (
          ...
          /* ── Read Aloud + Podcast controls (AI Summary tab) ── */
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            {raPhase === 'idle' && (
              <>
                ...
                <button onClick={onPodcast}
                  style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 10, border: '1.5px solid #8B5CF6', background: '#F5F0FF', color: '#6D28D9', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
                  🎙️ Podcast
                </button>
              </>
            )}
            {raPhase === 'preloading' && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 14px', borderRadius: 10, background: 'var(--bg-tint)', border: '1.5px solid var(--line)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--ink-2)' }}>
                    Preparing audio {raPreloadReady}/{documentReading.topics.length}…
                  </span>
                  <div style={{ width: 120, height: 3, borderRadius: 2, background: 'var(--line)', overflow: 'hidden' }}>
                    <div style={{ height: '100%', borderRadius: 2, background: 'var(--brand)', width: `${documentReading.topics.length > 0 ? (raPreloadReady / documentReading.topics.length) * 100 : 0}%`, transition: 'width 0.3s ease' }} />
                  </div>
                </div>
                <button onClick={raStop} style={{ fontSize: 11, color: 'var(--ink-4)', background: 'none', border: 'none', cursor: 'pointer', padding: '0 2px' }}>✕</button>
              </div>
            )}
            {raPhase === 'loading' && (
